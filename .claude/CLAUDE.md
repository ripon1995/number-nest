...
frontend/   React app (Vite + TypeScript)
  ...
  src/store/        Zustand stores: authStore (teacher session), courseStore (course list + create/update/delete),
                     ...
                     create/delete — no update, matching the backend's add/delete-only semantics)
  src/pages/        route pages: Login, Register, Dashboard, Courses, Students, Enrollments (all implemented);
                     Payments, Attendance (placeholders)
  ...
  src/types/        auth.ts, course.ts (incl. CourseDetail), student.ts, enrollment.ts — request/response shapes
                     matching the backend Pydantic schemas
