...
backend/
  app/
    ...
    exams/          implemented: models (Exam: course_id, exam_datetime, description, exam_mark), schemas,
                     repository (ExamRepository), service (ExamService — create/list/delete, NotFoundException
                     if course_id doesn't exist), router (/exams — POST/GET/DELETE, add/delete only, no edit-in-place)
...
frontend/   React app (Vite + TypeScript)
  ...
  src/pages/        route pages: Login, Register, Dashboard, Courses, Students, Enrollments, Payments, Attendance,
                     Exams, Notices (all implemented)
  ...
  src/pages/exams/    ExamsPage's building blocks: ExamTable (list, resolves course_id to a name via a lookup map),
                      ExamFormDialog (create only — no edit, course <select> + datetime-local + description
                      textarea + exam_mark number input, renders in the default-width Modal), ExamIcons (inline
                      SVGs, trash only), examDisplay.ts (formatDateTime helper), exams.css (styles shared by the above)
  ...
  src/types/        auth.ts, course.ts (incl. CourseDetail), student.ts, enrollment.ts, payment.ts, attendance.ts,
                     exam.ts, notice.ts — request/response shapes matching the backend Pydantic schemas
  src/api/          client.ts (fetch wrapper + auth headers), index.ts (barrel re-export), plus one file per
                     ...
                     exams.ts, notices.ts) — fetch helpers for the backend
docs/       project docs
```
...
1. **Course** — create/manage courses (name, recurring fee, one-time enrollment fee, subject, days, capacity, motto); course detail includes the list of enrolled students
...
6. **Exam** — schedule exams for a course (course, date/time, description, exam mark); add/delete only, not tied to individual enrollments
...
8. **Teacher** — the single teacher is the sole system user/operator; adds/manages students and courses
