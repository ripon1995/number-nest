...
frontend/   React app (Vite + TypeScript)
  ...
  src/pages/students/  StudentsPage's building blocks: StudentTable (list, rows carry a detail icon navigating to
                        /students/:id), StudentFormDialog (create/edit, renders in a Modal), StudentDetailPage (full
                        page, route /students/:id — student info + enrollments card, due-payments card, and
                        payment-history card, fetched via api.getStudent then enrollmentStore/courseStore/
                        paymentStore), studentDetailDisplay.ts (buildDuePayments — the due-month calculation),
                        StudentIcons (inline SVGs, incl. EyeIcon for the detail action), students.css (styles shared
                        by all of the above)
