...
backend/
  app/
    ...
    expenses/       implemented: models (Expense: category enum (contract_fare/asset/salary/utility/other),
                     amount, expense_date, plus nullable month/staff_name/direction used only by specific
                     categories), schemas (ExpenseCreate's model_validator enforces which fields each category
                     requires), repository (ExpenseRepository, incl. get_contract_fare_for_month/
                     get_salary_for_staff_month for the conflict checks below), service (ExpenseService —
                     create/list/delete, ConflictException if a contract_fare/salary expense already exists
                     for that month), router (/expenses — POST/GET/DELETE, add/delete only, no edit-in-place).
                     Two partial unique indexes (month unique within contract_fare; (staff_name, month) unique
...
frontend/   React app (Vite + TypeScript)
  ...
  src/pages/expenses/  ExpensesPage's building blocks: ExpenseTable (list, a Details column resolves the
                       ...
                       contract_fare, "staff — month" for salary, "Purchased/Sold: item" for asset, description
                       otherwise; optional emptyMessage prop for the filtered-empty case), ExpenseFormDialog
                       (create only — no edit, a category <select> plus amount/expense_date always shown and
                       month/staff_name/direction/description shown conditionally per category, mirroring the
                       backend's ExpenseCreate validator field-for-field, renders in the default-width Modal via
                       an expense-modal class), ExpenseIcons (inline SVGs, trash only), expenseDisplay.ts
                       (CATEGORY_OPTIONS, formatCategory/formatAmount/formatMonth/monthInputToApi/expenseDetails
                       helpers), expenses.css (styles shared by all of the above, incl. .expense-filters for the
                       category/month filter bar). ExpensesPage itself owns the filter state and filtering logic
                       (client-side, no backend query params) — category matches directly, month matches
                       against expense_date's YYYY-MM prefix (not the category-specific month field, which only
                       contract_fare/salary set) — same pattern as every other feature's filter bar.
...
11. **Expense tracking** — record institutional costs across five categories (monthly contract fare, asset purchase/sell, staff salary, utility costing, other costing); add/delete only. Not tied to students/courses/enrollments — a flat cost log, not part of the enrollment side of the domain. Contract fare and salary are capped at one entry per month (per staff member, for salary); the other three categories can repeat freely within a month.
