...
frontend/   React app (Vite + TypeScript)
  ...
  src/components/   shared components: Header, NavMenu, ProtectedRoute, Modal (generic backdrop+dialog, Escape-to-close,
                     optional className prop for per-use width overrides), ErrorDialog (renders ApiError via
                     Modal-like backdrop)
  src/store/        Zustand stores: authStore (teacher session), courseStore (course list + create/update/delete),
                     ...
                     create/delete — no update, matching the backend's add/delete-only semantics), paymentStore
  ...
  src/pages/enrollments/  EnrollmentsPage's building blocks: EnrollmentTable (list, resolves student/course ids to
                          names via lookup maps passed in as props), EnrollmentFormDialog (create only — no edit,
