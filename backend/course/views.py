...
from .serializers import CourseCreateSerializer, CourseRetrieveSerializer, CourseUpdateSerializer
...
class CourseRetrieveUpdateDestroyAPIView(RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return success_response(data=serializer.data, message="Course updated successfully")

    def destroy(self, request, *args, **kwargs):
        course = self.get_object()
        course.delete()
        return success_response(message="Course deleted successfully", status_code=status.HTTP_204_NO_CONTENT)
